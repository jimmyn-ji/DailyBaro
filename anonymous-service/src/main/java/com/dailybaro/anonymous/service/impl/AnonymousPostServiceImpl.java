package com.dailybaro.anonymous.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.dailybaro.anonymous.mapper.AnonymousPostMapper;
import com.dailybaro.anonymous.mapper.PostCommentMapper;
import com.dailybaro.anonymous.mapper.PostLikeMapper;
import com.dailybaro.anonymous.mapper.CommentMapper;
import com.dailybaro.anonymous.model.AnonymousPost;
import com.dailybaro.anonymous.model.PostLike;
...
import com.dailybaro.anonymous.service.AnonymousPostService;
...
public class AnonymousPostServiceImpl implements AnonymousPostService {
    ...
    @Override
    public Result<List<CommentVO>> getComments(Long postId, Long userId) {
        // 检查帖子是否存在
        if (anonymousPostMapper.selectById(postId) == null) {
            return Result.fail("帖子不存在");
        }
    ...
    @Override
    public Result<CommentVO> addComment(Long postId, CreateCommentDTO commentDTO, Long userId) {
        // 检查帖子是否存在
        if (anonymousPostMapper.selectById(postId) == null) {
            return Result.fail("帖子不存在");
        }
        ...
        return Result.success(convertCommentToVO(comment));
    }

    private CommentVO convertCommentToVO(Comments comment) {
    ...
    private AnonymousPostVO convertToVO(AnonymousPost post, Long currentUserId) {
        ...
        // 修正时间戳
        vo.setCreateTime(post.getCreateTime() != null ? post.getCreateTime().getTime() : null);
