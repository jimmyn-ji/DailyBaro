...
public class DiaryServiceImpl implements DiaryService {
    ...
    public Result<DiaryVO> createDiary(CreateDiaryDTO createDiaryDTO, Long userId) {
        ...
        // 4. 写日记奖励 +1 能量
        try {
            userMapper.increaseEnergy(userId, 1);
    ...
    public Result<DiaryVO> getDiaryById(Long diaryId) {
        ...
        try {
            com.project.model.User user = userMapper.getMyInfo(diary.getUserId());
            if (user != null) {
    ...
    public Result<List<DiaryVO>> findDiaries(QueryDiaryDTO queryDiaryDTO, Long userId) {
        ...
        // 关键词搜索
        if (queryDiaryDTO != null && queryDiaryDTO.getKeyword() != null && !queryDiaryDTO.getKeyword().trim().isEmpty()) {
            String keyword = queryDiaryDTO.getKeyword().trim();
