...
import defaultUserAvatar from '@/static/imgs/avatar.png'
...
const loadUserInfo = async () => {
  ...
  try {
    const res = await request.get(`/app/users/getMyInfo/${uid}`)
...
const sendMessage = async () => {
  ...
  try {
    // 获取用户ID，如果存在则使用RAG增强的接口
    const uid = window.sessionStorage.getItem('uid')
    const url = uid ? '/app/ai/chat' : '/app/ai/query'
    
    const { data } = await aiRequest.post(url, { question: inputMessage.value })
    if (data.code === 200) {
    ...
    if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
      ElMessage.error('AI响应超时，请稍后重试。如果问题持续，请检查网络连接或稍后再试。')
    } else {
