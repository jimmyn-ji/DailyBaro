...
public interface UserMapper {
    ...
    // 根据邮箱查询用户
    @Select("SELECT * FROM user WHERE email = #{email} AND isdelete = 0")
    User selectByEmail(String email);

    // 根据QQ openid查询用户
    @Select("SELECT * FROM user WHERE qq_openid = #{openid} AND isdelete = 0")
    User selectByQQOpenid(String openid);

    // 插入用户(需包含wx_openid字段)
    // 注意：实际使用 UserMapper.xml 中的定义，这里保留作为参考
    // XML 定义包含：account, password, wx_openid, phone, email, status, isdelete, energy
